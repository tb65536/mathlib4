...
variable {A B : Type*} [SetLike A B] {cl : Set B → A}
...
variable (cl) in
class IsClosureOperator : Prop where
  gc : GaloisConnection cl ((↑) : A → Set B)
  le_closure : ∀ (a : A), a ≤ cl (a : Set B)

variable [IsClosureOperator cl]

protected def IsClosureOperator.gi : GaloisInsertion cl ((↑) : A → Set B) :=
  gc.toGaloisInsertion le_closure

theorem closure_le {s : Set B} {a : A} : cl s ≤ a ↔ s ⊆ a :=
  IsClosureOperator.gc.le_iff_le

theorem subset_closure {s : Set B} : s ⊆ cl s :=
  IsClosureOperator.gc.le_u_l s

theorem closure_monotone : Monotone cl :=
  IsClosureOperator.gc.monotone_l

theorem closure_mono {s t : Set B} (h : s ⊆ t) : cl s ≤ cl t :=
  closure_monotone h

theorem mem_closure_of_mem {s : Set B} {b : B} (hx : b ∈ s) : b ∈ cl s :=
  subset_closure hx
...
variable [PartialOrder A] [IsClosureOperator cl]

theorem closure_eq (a : A) : cl (a : Set B) = a :=
  IsClosureOperator.gi.l_u_eq a

theorem closure_empty [OrderBot A] : cl (∅ : Set B) = ⊥ :=
  IsClosureOperator.gc.l_bot

theorem closure_univ [OrderTop A] : cl (Set.univ : Set B) = ⊤ :=
  IsClosureOperator.gi.l_top

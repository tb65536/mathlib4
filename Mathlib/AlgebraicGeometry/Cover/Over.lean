...
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
...
pullback in `Scheme`. -/
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
def Cover.pullbackCoverOver [𝒰.Over S] : W.Cover (precoverage P) where
  I₀ := 𝒰.I₀
...
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
instance (j : 𝒰.I₀) [𝒰.Over S] : ((𝒰.pullbackCoverOver S f).X j).Over S where
  hom := (pullback (f.asOver S) ((𝒰.f j).asOver S)).hom

@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
instance [𝒰.Over S] : (𝒰.pullbackCoverOver S f).Over S where
...
pullback in `Scheme`. -/
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
def Cover.pullbackCoverOverProp [𝒰.Over S] (hQ : ∀ j, Q (𝒰.X j ↘ S)) : W.Cover (precoverage P) where
  I₀ := 𝒰.I₀
...
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
instance (j : 𝒰.I₀) [𝒰.Over S] (hQ : ∀ j, Q (𝒰.X j ↘ S)) :
    ((𝒰.pullbackCoverOverProp S f hX hW hQ).X j).Over S where
  hom := (pullback (f.asOverProp (hX := hW) (hY := hX) S)
    ((𝒰.f j).asOverProp (hX := hQ j) (hY := hX) S)).hom

@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
instance [𝒰.Over S] (hQ : ∀ j, Q (𝒰.X j ↘ S)) : (𝒰.pullbackCoverOverProp S f hX hW hQ).Over S where
  isOver_map j :=
...
fiber products flipped. -/
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
def Cover.pullbackCoverOverProp' [𝒰.Over S] (hQ : ∀ j, Q (𝒰.X j ↘ S)) :
    W.Cover (precoverage P) where
  I₀ := 𝒰.I₀
...
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
instance [𝒰.Over S] (hQ : ∀ j, Q (𝒰.X j ↘ S)) :
    (𝒰.pullbackCoverOverProp' S f hX hW hQ).Over S where
  isOver_map j :=
...
@[deprecated "Use `Cover` in the `Over` category instead." (since := "2026-07-08")]
instance (j : (𝒰.bind 𝒱).I₀) [𝒰.Over S] [∀ x, (𝒱 x).Over S] : ((𝒰.bind 𝒱).X j).Over S :=

...
instance : IsClosureOperator (Subsemigroup.closure : Set M → Subsemigroup M) where
  le_closure _ := le_sInf fun _ ↦ id
  gc x y := by
    refine ⟨?_, fun h m hm ↦ mem_sInf.mp hm y h⟩
    intro h
    refine .trans ?_ h
    exact fun m hm ↦ mem_sInf.mpr fun z hz ↦ hz hm

@[deprecated (since := "2026-07-01")] alias mem_closure := _root_.mem_closure
